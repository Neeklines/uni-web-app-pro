...
function Dashboard() {
    ...
    const categoryTotals = Object.entries(categories).map(([label, stats]) => ({
        label,
        value: stats.total,
    })).sort((a, b) => b.value - a.value);

    const categoryCounts = Object.entries(categories).map(([label, stats]) => ({
        label,
        value: stats.count,
    })).sort((a, b) => b.value - a.value);
        ...
        <>
            ...
            <div className="bg-gray-900 text-white px-4 py-8 sm:px-6 sm:py-12">
                <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    {(categoryTotals.length > 0 || categoryCounts.length > 0) && (
